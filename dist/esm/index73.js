import { TopicCreateTransaction, TopicId, TopicMessageSubmitTransaction, TopicDeleteTransaction, TopicUpdateTransaction, KeyList, AccountId } from "@hashgraph/sdk";
import { Buffer } from "buffer";
import { BaseServiceBuilder } from "./index70.js";
const DEFAULT_AUTORENEW_PERIOD_SECONDS = 7776e3;
const MAX_SINGLE_MESSAGE_BYTES = 1e3;
class HcsBuilder extends BaseServiceBuilder {
   ...
   */
  submitMessageToTopic(params) {
    const topicId = typeof params.topicId === "string" ? TopicId.fromString(params.topicId) : params.topicId;
    const messageContents = params.message;
    const messageBytesLength = typeof messageContents === "string" ? Buffer.from(messageContents, "utf8").length : messageContents.length;
    ...
    let transaction = new TopicMessageSubmitTransaction().setTopicId(topicId).setMessage(messageContents);
    if (params.maxChunks) {
  ...
  async updateTopic(params) {
    ...
    if (Object.prototype.hasOwnProperty.call(params, "exemptAccountIds")) {
      if (params.exemptAccountIds && params.exemptAccountIds.length > 0 && !this.kit.signer.mirrorNode) {
        this.logger.warn(

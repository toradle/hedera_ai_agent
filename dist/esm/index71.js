import { AccountCreateTransaction, Hbar, Long, TransferTransaction, AccountId, AccountUpdateTransaction, AccountDeleteTransaction, AccountAllowanceApproveTransaction, TokenId, NftId, AccountAllowanceDeleteTransaction, ScheduleId, ScheduleSignTransaction } from "@hashgraph/sdk";
import BigNumber from "bignumber.js";
import { BaseServiceBuilder } from "./index70.js";
import { detectKeyTypeFromString } from "./index82.js";
const DEFAULT_ACCOUNT_AUTORENEW_PERIOD_SECONDS = 7776e3;
class AccountBuilder extends BaseServiceBuilder {
  ...
  createAccount(params) {
    ...
    if (typeof params.key !== "undefined") {
      if (params.key === null) {
        ...
        transaction.setKeyWithoutAlias(params.key);
      }
    }
    ...
    if (typeof params.autoRenewPeriod !== "undefined") {
      ...
      } else if (typeof params.autoRenewPeriod === "number" || params.autoRenewPeriod instanceof Long) {
        transaction.setAutoRenewPeriod(params.autoRenewPeriod);
        autoRenewPeriodSetByUser = true;
      } else if (typeof params.autoRenewPeriod === "object" && typeof params.autoRenewPeriod.seconds === "number") {
        transaction.setAutoRenewPeriod(
          params.autoRenewPeriod.seconds
        );
        autoRenewPeriodSetByUser = true;
      } else {
        this.logger.warn(
  ...
  transferHbar(params, isUserInitiated = true) {
    ...
    if (isUserInitiated && this.kit.userAccountId && this.kit.operationalMode === "returnBytes" && params.transfers.length === 1) {
  ...
  deleteAccount(params) {
    ...
    const transaction = new AccountDeleteTransaction().setAccountId(params.deleteAccountId).setTransferAccountId(params.transferAccountId);
    this.setCurrentTransaction(transaction);
    return this;
  }
  /**
   ...
   * @returns {this} The builder instance for chaining.
   */
  approveHbarAllowance(params) {
    const transaction = new AccountAllowanceApproveTransaction().approveHbarAllowance(
   ...
   */
  approveTokenNftAllowance(params) {
   ...
   * @returns {this} The builder instance for chaining.
   */
  approveFungibleTokenAllowance(params) {
    const tokenId = typeof params.tokenId === "string" ? TokenId.fromString(params.tokenId) : params.tokenId;
    let amountLong;
   ...
   */
  deleteNftSpenderAllowance(params) {
    const nftId = typeof params.nftId === "string" ? NftId.fromString(params.nftId) : params.nftId;
    const owner = params.ownerAccountId || this.kit.signer.getAccountId();
    const transaction = new AccountAllowanceDeleteTransaction().deleteAllTokenNftAllowances(
      nftId,
      owner
    );
    if (typeof params.memo !== "undefined") {
   ...
   * @returns {this} The builder instance for chaining.
   */
  revokeFungibleTokenAllowance(params) {
    const tokenId = typeof params.tokenId === "string" ? TokenId.fromString(params.tokenId) : params.tokenId;
    const transaction = new AccountAllowanceApproveTransaction().approveTokenAllowance(
      tokenId,
      params.ownerAccountId || this.kit.signer.getAccountId(),
   ...
   * @returns {this} The builder instance for chaining.
   */
  deleteNftSerialAllowancesForAllSpenders(params) {
    let ownerAccId;
   ...
   * @returns {this} The builder instance for chaining.
   */
  deleteTokenNftAllowanceForSpender(params) {
    let ownerAccIdToUse;
    if (params.ownerAccountId) {
      ownerAccIdToUse = typeof params.ownerAccountId === "string" ? AccountId.fromString(params.ownerAccountId) : params.ownerAccountId;
    } else {
      ownerAccIdToUse = this.kit.signer.getAccountId();
    }
    const sdkTokenId = typeof params.tokenId === "string" ? TokenId.fromString(params.tokenId) : params.tokenId;
    const sdkSerials = params.serials.map(
      (s) => {
   ...
   * @returns {this} The builder instance for chaining.
   */
  prepareSignScheduledTransaction(params) {
